...
ospf_check_abr_status (struct ospf *ospf)
{
  ...
  switch (ospf->abr_type)
	...
	UNSET_FLAG (new_flags, OSPF_FLAG_ABR);
      break;
    //default is this case, add abr_active_phase() here
    case OSPF_ABR_CISCO:
      if ((areas_configured > 1) && bb_act_attached && is_abr_act_phase())
	      SET_FLAG (new_flags, OSPF_FLAG_ABR);
      else
	      UNSET_FLAG (new_flags, OSPF_FLAG_ABR);
      break;
  ...
  if (new_flags != ospf->flags)
    ...
    if(MY_DEBUG)
      zlog_debug("in func ospf_check_abr_status,ospf->abr_type=%d,bb_act=%d,area_act=%d,bb=%d,area=%d,new_flag=0x%x",ospf->abr_type,bb_act_attached,areas_act_attached,bb_configured,areas_configured,new_flags);
}
