...
nsm_packet_received (struct ospf_neighbor *nbr)
{
  ...
  if(nbr->state == NSM_Leaving)
  {
    if(nbr->leaving_flag == 1)
      return NSM_Leaving;
    else
    {
      OSPF_NSM_TIMER_OFF (nbr->t_inactivity);
...
//oa state change is also included here 
static int 
...
} NSM [OSPF_NSM_STATE_MAX][OSPF_NSM_EVENT_MAX] =
{
  ...
  { /*Leaving: */
		{ NULL,                    NSM_DependUpon }, /* NoEvent           */
		{ nsm_packet_received,     NSM_DependUpon }, /* PacketReceived    */
...
nsm_change_state (struct ospf_neighbor *nbr, int state)
{
  ...
  if(state == NSM_Exchange && old_state == NSM_Leaving)
  {
    if(MY_DEBUG)
