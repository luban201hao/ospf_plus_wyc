...
ospf_ls_upd_timer (struct thread *thread)
{
  ...
  if(MY_DEBUG2)
    zlog_debug("in func ospf_ls_upd_timer, ???????????");
  ...
  if (ospf_ls_retransmit_count (nbr) > 0)
    {
      ...
      {
        if(MY_DEBUG2)
          zlog_debug("send upd in ospf_packet.c ospf_ls_upd_timer, update has %d lsas", listcount (update));
	     ...
	     struct stream *s, struct ospf_interface *oi, u_int16_t size)
{
  ...
  zlog_debug("in ospf_ls_upd, sz = %d, begin time = %ld.%ld", iph->ip_len, help_time.tv_sec, help_time.tv_usec);
#endif
  struct ospf_neighbor *nbr;
  struct list *lsas;
  struct listnode *node, *nnode;
...
#ifdef MY_DEBUG3
  quagga_gettime (QUAGGA_CLK_MONOTONIC, &help_time);
  zlog_debug("in ospf_ls_upd, end = %ld.%ld", help_time.tv_sec, help_time.tv_usec);
#endif
}
  ...
  while ((node = listhead (update)) != NULL)
    {
      ...
      if(MY_DEBUG1)
        zlog_debug("in ospf_make_ls_upd, adv = %x, id = %x", lsa->data->adv_router.s_addr, lsa->data->id.s_addr);
...
ospf_ls_upd_send_queue_event (struct thread *thread)
{
  ...
  if(MY_DEBUG1)
    zlog_debug("in ospf_ls_upd_send_queue_event, sending!!!!!!!!");
