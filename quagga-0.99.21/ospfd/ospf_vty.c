    ...
    for(rn=route_top(oi->nbrs); rn;  rn=route_next(rn))
    {
      ...
      if((nbr=(struct ospf_neighbor *)rn->info))
      {
        ...
        if(nbr->router_id.s_addr == nbr_addr.s_addr)
        {
          ...
          nbr->leaving_flag = 0;
          OSPF_ISM_EVENT_EXECUTE(nbr->oi,ISM_PredictedLinkUp);
        }
      }
    }
  }
      ...
      "add phase in cycle\n")
{
  ...
  zlog_debug("in func add_phase,now phase=%d,begin time at %ld.%ld ##########", global_phase, help_time.tv_sec, help_time.tv_usec);
  ...
  for(ALL_LIST_ELEMENTS(neighbor_info_list_cur->n_list,node,nnode,nbr_info))
  {
    ...
    if(nbr_info->phase_info[global_phase] == 0 && nbr_info->phase_info[last_phase] == 1)
    {
      ...
      predicted_linkup_sub(nbr_info->router_id);
    }
  }
  //should not use timer here, or will delay
      ...
      "begin_running\n")
{
  struct timeval help_time;
  quagga_gettime (QUAGGA_CLK_MONOTONIC,&help_time);
    zlog_debug("in func begin_running, time  = %ld.%ld", help_time.tv_sec, help_time.tv_usec);
  struct ospf *ospf = ospf_lookup();
    //do predicted linkdown and begin testing for predicted link up
  struct listnode *node, *nnode;
  struct neighbor_info *nbr_info;
...
// 此处添加的if_se相当于一个stub网段，其路由也需要被计算。
// 在ifconfig重新设置接口IP地址后，需要对于路由信息重新发布
DEFUN(if_se,
      if_se_cmd,
      "if_se area A.B.C.D",
      "if_se area A.B.C.D\n")
{
  //printf("if_se 1,argv[0]=%s\n",argv[0]);
  struct interface *ifp = vty->index;
  struct ospf_interface *oi;
  ...
  inet_aton(argv[0],&area_addr);
  if(MY_DEBUG)
    zlog_debug("in func if_se, area_addr=%x", area_addr.s_addr);
  //printf("in func if_se,area_addr=%x\n",area_addr.s_addr);
  for(ALL_LIST_ELEMENTS_RO (ifp->connected, cnode, co))
  {
    ...
    {
      if(MY_DEBUG)
        zlog_debug("in func if_se  111");
      //printf("if_se 3,co->addr=%x\n",co->address->u.prefix4.s_addr);
      ...
      oi->output_cost = 10;
      oi->area = ospf_area_get(ospf,area_addr,OSPF_AREA_ID_FORMAT_ADDRESS);
...
DEFUN(read_abr_info,
      read_abr_info_cmd,
      "read_abr_info FILENAME",
      "read_abr_info FILENAME\n")
{
  if(MY_DEBUG)
    zlog_debug("in func read_abr_info");
  DATA_INFO_STR data_info_str;
  fileToMatrix_str(argv[0],&data_info_str);

  int phase_count=data_info_str.line_total[0];
  abr_phase_info=(int *)calloc(phase_count,sizeof(int));
  if(MY_DEBUG)
    zlog_debug("data_info_str.row=%d,phase_count=%d",data_info_str.row_total,phase_count);
  
  for(int i=0;i<phase_count;++i)
  {
    
    abr_phase_info[i]=atoi(data_info_str.matrix[0][i]);
    if(MY_DEBUG)
      zlog_debug("abr:%d",abr_phase_info[i]);
  }
  return CMD_SUCCESS;
}
int is_abr_act_phase()
{
  if(MY_DEBUG)
    zlog_debug("in func is_abr_act_phase");
  if(abr_phase_info == NULL)
  {
    return 1;
  }
  if(abr_phase_info[global_phase] == 0)
  {
    return 1;
  }
  return 0;
...
{
  install_element (VIEW_NODE,&read_abr_info_cmd);
  install_element (ENABLE_NODE,&read_abr_info_cmd);
  install_element (CONFIG_NODE,&read_abr_info_cmd);
