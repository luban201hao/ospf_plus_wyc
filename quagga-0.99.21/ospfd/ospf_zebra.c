                            ...
                            zebra_size_t length)
{
  ...
  zlog_debug("in func ospf_interface_address_add, time = %ld.%ld", help_time.tv_sec, help_time.tv_usec);
    ...
    for(rn=route_top(IF_OIFS(c->ifp));rn;rn=route_next(rn))
    {
      if(oi=(struct ospf_interface *)rn->info)
      {
        if(oi->type==OSPF_IFTYPE_SE)
        {
          ...
          //LSA should up date here
          ospf_router_lsa_update_area (oi->area);
          if(MY_DEBUG)
            zlog_debug("in ospf_interface_address_add, se router lsa has update");
        }
      }
    }
  }
                               ...
                               zebra_size_t length)
{
  ...
  struct timeval help_time;
  quagga_gettime (QUAGGA_CLK_MONOTONIC, &help_time);
  zlog_debug("in func ospf_interface_address_delete, time = %ld.%ld", help_time.tv_sec, help_time.tv_usec);
